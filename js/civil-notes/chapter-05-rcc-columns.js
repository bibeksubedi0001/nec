(function () {
    ...
    Object.assign(registry, {
        ACiE0504: {
            code: "ACiE0504",
            questionCount: 46,
            formulaSheet: "<table><thead><tr><th scope='col'>Quantity</th><th scope='col'>Relation and basis</th></tr></thead><tbody><tr><th scope='row'>Minimum eccentricity</th><td>e<sub>min</sub> = max(l/500 + D/30, 20 mm).</td></tr><tr><th scope='row'>Short-column axial capacity</th><td>P<sub>u</sub> = 0.4f<sub>ck</sub>A<sub>c</sub> + 0.67f<sub>y</sub>A<sub>sc</sub>, A<sub>c</sub> = A<sub>g</sub> − A<sub>sc</sub>; valid for e ≤ 0.05D.</td></tr><tr><th scope='row'>Longitudinal steel</th><td>0.008A<sub>g</sub> ≤ A<sub>sc</sub> ≤ 0.06A<sub>g</sub>.</td></tr><tr><th scope='row'>Tie pitch</th><td>s<sub>max</sub> = min(least lateral dimension, 16φ<sub>min</sub>, 300 mm).</td></tr><tr><th scope='row'>Helical spacing</th><td>max(25 mm, 3φ<sub>h</sub>) ≤ s ≤ min(75 mm, d<sub>core</sub>/6).</td></tr><tr><th scope='row'>Footing area</th><td>A<sub>req</sub> = N<sub>s</sub>/q<sub>allow</sub>; service load with compatible bearing pressure.</td></tr><tr><th scope='row'>Base pressure</th><td>q<sub>max,min</sub> = (P/A)(1 ± 6e/L); no tension if |e| ≤ L/6.</td></tr><tr><th scope='row'>Load resultant</th><td>x<sub>R</sub> = ΣP<sub>i</sub>x<sub>i</sub>/ΣP<sub>i</sub>.</td></tr><tr><th scope='row'>Trapezoidal footing</th><td>A = L(a + b)/2; x̄ = L(a + 2b)/[3(a + b)].</td></tr><tr><th scope='row'>Combined footing</th><td>V(x) = wx − ΣP<sub>i</sub>; M(x) = wx<sup>2</sup>/2 − ΣP<sub>i</sub>(x − a<sub>i</sub>).</td></tr><tr><th scope='row'>Footing minimum steel</th><td>A<sub>s,min</sub> = 0.0012bD in each direction.</td></tr></tbody></table>",
            blocks: [
                {
                    id: "column-actions-strains-and-units",
                    title: "Column actions, strain limits and consistent units",
                    html: "<p>A reinforced-concrete column transfers axial force and usually bending moments between floors and foundations. Bond makes the longitudinal steel and surrounding concrete deform compatibly; transverse reinforcement restrains the longitudinal bars and confines the core. </p><p>The teaching basis here is <strong>IS 456:2000, Fourth Revision, with the relevant appended amendments</strong>, and <strong>IS 1343:2012, Second Revision</strong>, read with the identified 2019 amendment. </p><p>For the IS 456 limit-state idealization of <strong>pure axial compression</strong>, the maximum concrete strain is <strong>ε<sub>c</sub> = 0.002</strong>. The familiar <strong>0.0035</strong> is the extreme compression-fibre limit in bending. Uniform axial strain and a bending strain gradient are different states; a member under combined axial force and bending, including an entirely compressed section, needs the applicable strain-compatibility conditions of §39.1. Neither number is a universal limit for every compression state.</p><table><thead><tr><th scope='col'>Quantity</th><th scope='col'>Consistent units and meaning</th></tr></thead><tbody><tr><th scope='row'>P, M, A, I, Z</th><td>For section stresses use N, N·mm, mm<sup>2</sup>, mm<sup>4</sup>, mm<sup>3</sup>; Z = I/c.</td></tr><tr><th scope='row'>f<sub>ck</sub>, f<sub>y</sub>, σ</th><td>N/mm<sup>2</sup> = MPa; characteristic material strengths are not applied stresses.</td></tr><tr><th scope='row'>Footing pressure q</th><td>kN/m<sup>2</sup> = kPa; 1000 kPa = 1 MPa.</td></tr><tr><th scope='row'>ε and moment conversion</th><td>Strain is dimensionless; 1 kN·m = 10<sup>6</sup> N·mm.</td></tr></tbody></table>",
                    moreHtml: "<p>An axial force does not prove that bending is absent: frame action, imperfect alignment and second-order displacement can all create moment.</p><p>These are edition-labelled study references, not a claim about the latest standards or legal acceptance in Nepal. Worked illustrations explain mechanics rather than dimension an actual building.</p>",
                ...
                },
                {
                    ...
                    html: "<p><strong>IS 456:2000 §25.1.2</strong> classifies a compression member as short only when <strong>l<sub>ex</sub>/D &lt; 12 and l<sub>ey</sub>/b &lt; 12</strong>. The two effective lengths and corresponding lateral dimensions belong to their respective bending planes. If either ratio reaches 12, the member does not meet this short-column definition. This effective-length-to-dimension test is not the Euler slenderness ratio l<sub>e</sub>/r, where r is radius of gyration.</p><p>Write l<sub>e</sub> = Kl, with l the unsupported length. Annex E/Table 28 recommends <strong>K = 0.65</strong> for the specified case with both ends effectively held in position and restrained against rotation. </p><p><strong>Bank calculation:</strong> a 400 × 500 mm section is <strong>rectangular, not square</strong>. With l = 5000 mm and the stated fully held, rotationally restrained interpretation in both planes, l<sub>ex</sub> = l<sub>ey</sub> = 0.65 × 5000 = 3250 mm. The two ratios are <strong>3250/400 = 8.125</strong> and <strong>3250/500 = 6.5</strong>. Both are below 12, so this interpretation gives a <strong>short column</strong>, not the stored long-column answer. If translational restraint is unknown, establish it rather than silently assigning K.</p>",
                    moreHtml: "<p>Confirm these restraints in each plane. <strong>Rotational fixity alone does not establish that frame sway is suppressed</strong>; the effective-length model must reflect translational restraint and the surrounding frame.</p>",
                ...
                },
                {
                    ...
                    html: "<p>Nominally concentric loading still needs an allowance for unavoidable imperfections. Under <strong>IS 456:2000 §25.4</strong>, the minimum eccentricity for the relevant plane is <strong>e<sub>min</sub> = max(l/500 + D/30, 20 mm)</strong>, using the <strong>unsupported</strong> length l and the corresponding lateral dimension D, both in millimetres. Do not insert the smaller effective buckling length in this expression.</p><table><thead><tr><th scope='col'>400 × 400 mm bank column</th><th scope='col'>Calculated allowance</th><th scope='col'>Adopted minimum</th></tr></thead><tbody><tr><th scope='row'>l = 5000 mm</th><td>5000/500 + 400/30 = 23.333… mm</td><td>23.333… mm</td></tr><tr><th scope='row'>l = 3000 mm</th><td>3000/500 + 400/30 = 19.333… mm</td><td>20 mm, because the lower bound governs</td></tr></tbody></table><p>For a given axial force, eccentricity produces a moment contribution P e; for example, 1000 kN acting 20 mm off-centre produces 20 kN·m. This is an illustrative force-times-distance conversion, not a replacement for the actual frame moments. </p>",
                    moreHtml: "<p>Calculate the relevant eccentricities and preserve the actual uniaxial or biaxial loading; apply the code's combination provisions rather than discarding larger analysed moments.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p><strong>IS 456:2000 §26.5.3.1(c–g)</strong> requires at least <strong>four longitudinal bars in a rectangular column</strong> and <strong>six in a circular column</strong>; longitudinal bar diameter must be <strong>at least 12 mm</strong>. In a helically reinforced column, the longitudinal bars are arranged against the inside of the helix, equidistant around it. </p><p>The <strong>300 mm maximum spacing</strong> in §26.5.3.1(g) is measured <strong>along the periphery of the column</strong>. It is not a universal maximum for the separation of any two parallel reinforcement bars anywhere in a structure. </p><p><em>Illustrative area check:</em> four 20 mm bars provide 4 × π × 20<sup>2</sup>/4 = <strong>1256.6 mm<sup>2</sup></strong>. That satisfies the minimum number for a rectangle but falls below 1280 mm<sup>2</sup> for an ordinary 400 mm square column. Counting bars without checking their total area misses this shortfall.</p>",
                    moreHtml: "<p>The minimum number is a starting detailing rule, not proof that every other requirement has been met.</p><p>Minimum clear gaps for concrete placement, peripheral spacing, total steel area and lateral restraint all require separate attention; a large section can need more than the minimum bar count.</p><p>Circular columns are used in buildings, piers and piles; they are not inherently forbidden or weak. A curved face can complicate beam formwork, bar anchorage and joint confinement compared with a rectangular face. Whether it is convenient or economical depends on the actual beam–column joint and construction method, not a blanket statement that circular columns are generally unused.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>Lateral ties hold the reinforcement cage in position and restrain compression bars against outward buckling. Their effectiveness depends on the tie arrangement and anchorage as well as their spacing. Under the general rule in <strong>IS 456:2000 §26.5.3.2</strong>, the maximum pitch is <strong>s<sub>max</sub> = min(least lateral dimension, 16φ<sub>min</sub>, 300 mm)</strong>, where φ<sub>min</sub> is the <strong>smallest longitudinal-bar diameter</strong> being considered.</p><table><thead><tr><th scope='col'>400 × 400 mm bank column</th><th scope='col'>Three competing limits</th><th scope='col'>Maximum pitch</th></tr></thead><tbody><tr><th scope='row'>20 mm longitudinal bars</th><td>400 mm; 16 × 20 = 320 mm; 300 mm</td><td>300 mm</td></tr><tr><th scope='row'>12 mm longitudinal bars</th><td>400 mm; 16 × 12 = 192 mm; 300 mm</td><td>192 mm</td></tr></tbody></table><p>If a cage contains both 20 and 12 mm longitudinal bars, selecting the 20 mm diameter would overlook the smaller bar's restraint requirement. A calculated maximum of 192 mm means the adopted spacing must not exceed it, not that ties must be set at exactly 192 mm. </p>",
                    moreHtml: "<p>These general maxima do not override closer spacing required in seismic confinement zones, joints, splice regions or a particular structural design.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>For a concentric preliminary bearing calculation, <strong>A<sub>req</sub> = N<sub>s</sub>/q<sub>allow</sub></strong> uses a service-level vertical resultant N<sub>s</sub> and an allowable pressure on the <strong>same load and gross/net basis</strong>. A gross contact check includes the appropriate column load, footing self-weight, soil above the footing and other relevant actions. </p><p><strong>Printed data:</strong> 330 kN divided by 150 kN/m<sup>2</sup> gives <strong>2.20 m<sup>2</sup></strong>. This is valid for the stated simplified calculation only if 330 kN is the compatible load to compare with that pressure, or if additional weight is explicitly neglected at the preliminary stage. The 200 × 400 mm column dimensions affect footing projections and structural checks but do not change this division.</p><p><em>Alternative illustrative assumption:</em> if 330 kN excludes footing/overburden weight, 150 kPa is a gross allowable pressure, and an estimated extra load of 10% is deliberately adopted, the total is 363 kN and the preliminary area is <strong>2.42 m<sup>2</sup></strong>. Ten percent is not a universal code addition. </p>",
                    moreHtml: "<p>Geotechnical net pressure commonly deducts the original overburden at founding level; use the actual definition in the soil report rather than assuming that net means column load only.</p><p>Replace an estimate with geometry-based weights, and never add it again to a load that already includes them.</p><p>Check eccentric pressure and settlement as well as average bearing. Structural bending and shear use the appropriate factored load combinations with a compatible factored reaction model; a factored column load must not simply be divided by a service allowable pressure and called a consistent design.</p>",
                ...
                },
                {
                    ...
                    html: "<p>First locate the resultant of the <strong>load system</strong>, not merely the geometric centre of a column. With vertical forces P<sub>i</sub> at x<sub>i</sub> and no applied couples, <strong>x<sub>R</sub> = ΣP<sub>i</sub>x<sub>i</sub>/ΣP<sub>i</sub></strong>. </p><p>For a rectangular base of length L in the eccentricity direction and width B, <strong>A = BL</strong>, <strong>Z = BL<sup>2</sup>/6</strong> and <strong>q<sub>max,min</sub> = P/A ± M/Z = (P/A)(1 ± 6e/L)</strong>, with e = M/P and compressive contact pressure positive. This linear, full-area-contact expression has no soil tension only when <strong>|e| ≤ L/6</strong> for uniaxial eccentricity. </p><p><em>Illustrative service case:</em> P = 600 kN, L = 3 m, B = 2 m and M = 150 kN·m give e = 0.25 m, A = 6 m<sup>2</sup>, Z = 3 m<sup>3</sup> and pressures <strong>150 and 50 kPa</strong>. </p>",
                    moreHtml: "<p>Include the appropriate dead loads and applied moments in the full equilibrium calculation. The soil-reaction resultant must be equal, opposite and collinear with the applied resultant for equilibrium, whether the pressure is uniform or nonuniform.</p><p>Uniform pressure follows from the <strong>idealized linear full-contact model</strong> when the applied resultant passes through the <strong>centroid of the footing area</strong> and the corresponding moments about that centroid vanish. Coincidence of the load and reaction resultants alone is not this extra condition. Even homogeneous soil can develop nonuniform contact stress because pressure also depends on soil constitutive behaviour, foundation stiffness and deformation compatibility.</p><p>If it predicts negative contact pressure, reassess the compression-only contact area and equilibrium; do not merely erase the negative part while leaving the other pressures unchanged.</p><p>The resultants still balance along the same line, but the pressure is plainly not uniform. Biaxial moments require both pressure gradients and corner-contact checks.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>A combined footing carries more than one column on a common base. When two column loads are unequal, a trapezoidal plan can move the area centroid toward the larger load by providing more width at that end. </p><p>For a trapezoidal plan, define <strong>a as the width at x = 0</strong>, <strong>b as the width at x = L</strong>, and L as the distance between these parallel ends, not automatically the column spacing. Then <strong>A = L(a + b)/2</strong> and the area centroid measured from the a end is <strong>x̄ = L(a + 2b)/[3(a + b)]</strong>. </p><p><em>Illustrative geometry:</em> a = 2 m, b = 4 m and L = 6 m give A = 18 m<sup>2</sup> and x̄ = <strong>3.333… m from the 2 m-wide end</strong>. Compare that location with the load resultant before assuming uniform pressure. </p>",
                    moreHtml: "<p>It is <strong>one possible solution</strong>, not a unique consequence of unequal loading. A rectangular footing with suitable position and projections, a strap arrangement or another foundation system may also satisfy the load path and site constraints.</p><p>Reversing the reference end reverses the roles of a and b. Check that the centroid moves toward the wider end.</p><p>Geometry alone does not establish allowable bearing, settlement, shear or reinforcement. For the adopted idealization, the pressure model and both force and moment equilibrium must remain compatible.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>After choosing a compatible pressure distribution, obtain shear and bending from a footing free body with <strong>soil reaction acting upward</strong> and <strong>column loads acting downward</strong>. This is an inverted-beam load arrangement, not a downward-loaded floor slab supported upward by columns. </p><p>For a longitudinal strip with x measured from its free left edge, take <strong>sagging M as positive, giving bottom tension</strong>, and define V = dM/dx. With upward line load w and downward point-column loads P<sub>i</sub> at a<sub>i</sub>, equilibrium gives <strong>V(x) = wx − ΣP<sub>i</sub></strong> for columns already crossed by the cut, and <strong>M(x) = wx<sup>2</sup>/2 − ΣP<sub>i</sub>(x − a<sub>i</sub>)</strong>. Here dV/dx = +w because w is explicitly upward. Include applied couples or other loads when present.</p><p><em>Illustrative factored net-load model:</em> use a 6 m-long, 2 m-wide strip with q<sub>u</sub> = 100 kPa upward, hence w = 200 kN/m, and two 600 kN downward column forces at x = 1 and 5 m. Uniform distributed dead load is assumed consistently netted against its matching reaction component; the gross bearing check is separate. Total upward and downward forces are both 1200 kN.</p><table><thead><tr><th scope='col'>Section x</th><th scope='col'>Moment from the stated free body</th><th scope='col'>Flexural tension face</th></tr></thead><tbody><tr><th scope='row'>1 m, first column line</th><td>100 × 1<sup>2</sup> = +100 kN·m</td><td>Bottom</td></tr><tr><th scope='row'>3 m, centre</th><td>100 × 3<sup>2</sup> − 600 × 2 = −300 kN·m</td><td>Top</td></tr><tr><th scope='row'>5 m, second column line</th><td>100 × 5<sup>2</sup> − 600 × 4 = +100 kN·m</td><td>Bottom</td></tr></tbody></table>",
                    moreHtml: "<p>Use the resulting positive and negative moment envelopes to place reinforcement; maximum bending moment is important but does not replace shear, punching or geotechnical checks.</p><p>M and V return to zero at the right free edge. This example has top tension centrally and bottom tension in the outer regions; <strong>these are not universal locations</strong>. Actual column moments, overhangs, pressure variation and load reversal govern. Point columns are only a global-analysis idealization: use finite column faces, actual critical sections, anchorage and transverse action for detailing.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>Footing depth must satisfy several mechanisms. Flexure develops over the projections beyond a column; one-way shear can separate an outer strip; punching can drive the column through a surrounding perimeter. </p><table><thead><tr><th scope='col'>Check for an ordinary concrete column on a soil-supported footing</th><th scope='col'>Critical location</th></tr></thead><tbody><tr><th scope='row'>Bending, §34.2.3.2(a)</th><td>At the concrete column, pedestal or wall face.</td></tr><tr><th scope='row'>One-way shear, §34.2.4.1(a), Amendment 1</th><td>d from the column/pedestal/wall face on soil.</td></tr><tr><th scope='row'>Punching, §34.2.4.1(b)/§31.6.1</th><td>Ordinarily a perimeter offset d/2 from the loaded area; adapt it at edges and openings.</td></tr></tbody></table><p><em>Illustrative concentric case:</em> a 2 × 2 m footing supports a centred 0.4 × 0.4 m column, with d = 0.3 m, compatible uniform net upward q<sub>u</sub> = 200 kPa and column P<sub>u</sub> = 800 kN. The face projection is 0.8 m. A one-metre strip has face moment <strong>200 × 0.8<sup>2</sup>/2 = 64 kN·m/m</strong>; one-way shear at d from that face is <strong>200 × (0.8 − 0.3) = 100 kN/m</strong>.</p><p>The punching perimeter has side 0.4 + 0.3 = 0.7 m, perimeter u = 2.8 m and enclosed area 0.49 m<sup>2</sup>. Subtract the enclosed upward reaction: V<sub>p</sub> = 800 − 200 × 0.49 = <strong>702 kN</strong>. The nominal average stress is V<sub>p</sub>/(ud) = 835.714 kPa = <strong>0.836 MPa</strong>. </p><p><strong>§34.1.2</strong> requires edge thickness of at least <strong>150 mm on soil</strong> and <strong>300 mm above the tops of piles</strong> for pile-supported footings. The latter is not an unconditional 300 mm total pile-cap thickness. Pile-supported one-way shear has the separately specified d/2 location in the amended provision; do not substitute it for the d location on soil. Strength, cover and development requirements can demand greater thickness.</p>",
                    moreHtml: "<p>The <strong>effective depth d</strong> is measured to the relevant tensile reinforcement and is not the same as overall thickness D or the thickness of a sloping edge.</p><p>These are demand calculations, not a concrete-capacity or complete design check; moment transfer can make perimeter stresses nonuniform.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>For an ordinary footing governed by <strong>IS 456:2000 §34.5.1</strong>, the slab minimum-reinforcement provision of §26.5.2.1 applies. With high-strength deformed reinforcement such as Fe415, the minimum is <strong>0.12% of the gross concrete cross-sectional area in each direction</strong>. It is not 0.12% shared between two directions, nor a percentage of the effective-depth area or of the plan area.</p><p>For a strip width b and overall depth D, <strong>A<sub>s,min</sub> = 0.0012bD</strong>. <em>Illustrative strip:</em> b = 1000 mm and D = 400 mm give <strong>480 mm<sup>2</sup>/m in each direction</strong>. A 12 mm bar has area π × 12<sup>2</sup>/4 = 113.10 mm<sup>2</sup>; at 200 mm spacing it supplies approximately <strong>565.5 mm<sup>2</sup>/m</strong>. </p>",
                    moreHtml: "<p>This checks only the illustrated minimum-area arithmetic, not the flexural demand or a complete reinforcement layout.</p><p>Provide the reinforcement needed for the governing moment as well as the minimum. Bar spacing, cover, development into the column region and the footing ends, load transfer at the column interface, and any separate requirements for thick footings remain necessary. Whether top reinforcement is also required follows the actual moment envelope and detailing requirements, not the minimum percentage alone.</p>",
                ...
                },
                {
                    ...
                    html: "<p>A strap footing connects separate pads with a sufficiently stiff beam so that the system can balance an eccentric column action, often at a property boundary. Begin with column loads, pad-reaction locations and the system's force and moment equilibrium. </p><p><em>Illustrative moment contribution:</em> a 600 kN column force offset 0.3 m from its pad-reaction line contributes <strong>180 kN·m</strong> about that line. The complete pad-and-strap equilibrium determines how this is balanced; 180 kN·m is not, without that analysis, a finished strap design moment. </p>",
                    moreHtml: "<p>The strap transfers bending and shear between the pads; it is not merely a nominal tie added after sizing two unrelated isolated footings.</p><p>The usual simplified strap model assumes <strong>no significant soil reaction directly under the strap</strong>. That is a modelling and construction condition, not an automatic property of every beam between pads. If contact is allowed and materially contributes to support, include the distributed reaction and its compatibility with pad movement. Pad settlement and strap stiffness can change the force distribution.</p><p>Designers often revise trial pad sizes and strap stiffness after bearing, bending, shear and settlement checks. This explains the bank's iteration terminology, but <strong>trial and error is not a constitutive law or the only possible solution method</strong>.</p>",
                ...
                },
                {
                    ...
                    html: "<p>The assigned item supplies only <strong>q<sub>2</sub> = αq<sub>s</sub></strong> and asks for footing shape after setting the factor to zero. If q<sub>s</sub> is a finite pressure, direct substitution gives <strong>α = 0 implies q<sub>2</sub> = 0</strong>. </p><p>Pressure intensity and plan geometry are distinct quantities. Knowing one pressure value, even if it were identified as an edge pressure, does not establish the complete pressure field, contact area or base outline. <em>Illustrative substitution only:</em> q<sub>s</sub> = 150 kPa and α = 0 produce q<sub>2</sub> = 0 kPa; no plan dimensions follow from those two numbers.</p><p>To recover a determinate problem one would need the definition of α, the meaning and location of q<sub>2</sub> and q<sub>s</sub>, the other boundary pressures or pressure law, load resultants and the relation between dimensions and contact pressure. </p>",
                    moreHtml: "<p>It does not give a nonzero uniform pressure and contains no equation defining a rectangular plan. The stem also switches between a Latin a and Greek α without defining their roles.</p><p>Without that information, retain an explicit gap rather than inventing an alternative formula or choosing a footing outline from an unsupported explanation.</p>",
                ...
                },
                {
                    ...
                    html: "<p>Prestressing deliberately introduces a tendon force and balancing actions into concrete before the intended service loading. The initial compression and eccentric prestress moment can offset later tensile stresses and improve crack control. </p><table><thead><tr><th scope='col'>Process</th><th scope='col'>Sequence and initial force transfer</th></tr></thead><tbody><tr><th scope='row'>Pre-tensioning</th><td>Stress tendons against an external bed before casting. After the concrete reaches the required actual strength, release the tendons; bond transfers force into the concrete over a transmission length.</td></tr><tr><th scope='row'>Post-tensioning</th><td>Cast and cure the member, then jack the tendons against it. End anchorages transfer the force. Grouting a bonded duct subsequently provides bond and protection; unbonded systems retain a different bond condition.</td></tr></tbody></table>",
                    moreHtml: "<p>Unlike passive reinforcement, which develops additional stress as the member deforms under load, a prestressing tendon is intentionally stressed during construction. Prestressed members can also contain ordinary reinforcement for local, shear and crack-control duties.</p><p>High-strength steel permits useful prestress to remain after losses. Suitable concrete strength and quality are needed for compression, pre-tensioning bond transfer, and concentrated post-tensioning anchorage bearing. Anchorage zones also need checks for bursting and splitting, not simply a high concrete grade. Aggregate, mix, curing, age, creep and shrinkage matter; a higher nominal grade alone does not guarantee a particular loss or eliminate end-zone reinforcement.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>A loss of prestress reduces <strong>tendon force and the compression applied to the concrete</strong>. It does not, by definition, reduce the concrete's characteristic compressive strength f<sub>ck</sub>. </p><table><thead><tr><th scope='col'>Stage or mechanism</th><th scope='col'>What to account for</th></tr></thead><tbody><tr><th scope='row'>Pre-tensioning release</th><td>Concrete elastic shortening reduces tendon extension as force is transferred by bond; account separately for any earlier bed-stage loss.</td></tr><tr><th scope='row'>Post-tensioning operations</th><td>Duct friction acts during stressing and anchorage seating during lock-off. Later tendon stressing can shorten concrete and reduce force in tendons already anchored.</td></tr><tr><th scope='row'>Time-dependent effects in both methods</th><td>Concrete creep and shrinkage, and prestressing-steel relaxation, depend on time, stress history and material behaviour.</td></tr></tbody></table><p>At a specified section and stage, write <strong>P<sub>eff</sub> = P<sub>0</sub> − ΣΔP</strong>, defining the baseline P<sub>0</sub> before selecting losses. A jacking force is not automatically the force everywhere along a frictional tendon. </p><p><em>Illustrative remaining-loss calculation:</em> suppose P<sub>0</sub> = 1000 kN is already the force just after transfer at the section of interest. If the subsequently evaluated creep, shrinkage and relaxation force losses are 60, 40 and 50 kN on a consistent basis, then <strong>P<sub>eff</sub> = 850 kN</strong>. The 15% total is only this example's arithmetic; it is not a universal loss allowance, and immediate losses already included in P<sub>0</sub> are not subtracted again.</p>",
                    moreHtml: "<p>Reduced precompression can change cracking resistance, deflection and shear behaviour, but these member responses must not be confused with a change in the material's specified grade.</p><p>Do not deduct lock-off or elastic-shortening losses again if they are already included in a measured or calculated transfer force. Real loss components can interact, so maintain a consistent staged calculation rather than mixing percentages with different baselines.</p>",
                ...
                },
                {
                    ...
                    html: "<p>Tendon eccentricity varies along a member and produces a prestress moment opposing or adding to the external moment. For the simple sign convention above, the net sagging bending action is <strong>M<sub>external</sub>(x) − Pe(x)</strong>. </p><p>Let x run along the span and let the tendon ordinate <strong>y(x) be positive downward</strong> from the centroidal line. For small slopes and approximately constant tendon force P, the tendon force on the concrete per unit length, also downward-positive, is <strong>q<sub>t</sub>(x) = P d<sup>2</sup>y/dx<sup>2</sup></strong>. Thus negative q<sub>t</sub> means an upward equivalent load. End forces and anchorage eccentricities must accompany the distributed equivalent load for overall equilibrium.</p><p>For the illustrative parabola <strong>y(x) = 4e<sub>mid</sub>x(L − x)/L<sup>2</sup></strong>, with zero end eccentricity and downward midspan eccentricity e<sub>mid</sub>, y'' = −8e<sub>mid</sub>/L<sup>2</sup>. The upward balancing-load magnitude is consequently <strong>w<sub>bal</sub> = 8Pe<sub>mid</sub>/L<sup>2</sup></strong>. With P = 900 kN, e<sub>mid</sub> = 0.15 m and L = 6 m, this gives <strong>30 kN/m upward</strong>.</p>",
                    moreHtml: "<p>Its distribution, section stiffness EI and support conditions determine curvature and deflection. Cable profile is therefore a direct design variable, but aggregate-related modulus, concrete age, creep, shrinkage and prestress loss also influence deflection.</p><p>Matching a downward load by this upward equivalent action can cancel its bending in the stated idealization; it does not remove axial precompression or every other load-stage requirement. A changed profile, friction-varying P, continuity or losses changes the balance. Upward camber and downward service deflection must be assessed together, rather than concluding that the whole concrete section is always compressed or that cable shape alone fixes the final deflection.</p>",
            ...
            cautions: [
                {
                    "id": "check-desi787-00035",
                    "status": "review",
                    "prompt": "When two column loads are unequal, which of the possible footing can be provided?",
                    "html": "<p><strong>Unequal loads do not uniquely select a trapezoid.</strong> A trapezoidal, suitably located rectangular, or strap arrangement may be feasible. Column positions, property boundaries and the intended contact-pressure model determine the choice.</p>",
                    "sources": [
                        {
                            "id": "DESI787-00035",
                            "set": 5,
                ...
                {
                    "id": "check-desi787-00038",
                    "status": "review",
                    "prompt": "why circular column is generally not used?",
                ...
                {
                    "id": "check-desi787-00039",
                    "status": "review",
                    "prompt": "Prestress loss results in",
                ...
                {
                    "id": "check-desi787-00041",
                    "status": "review",
                    "prompt": "The minimum cube strength of concrete used for a pre-stressed member, is",
                    "html": "<p><strong>The older keyed strength lacks an edition and prestressing process.</strong> IS 1343:2012 uses M40 for pre-tensioned and M30 for post-tensioned work as characteristic-strength baselines. Transfer strength is a separate actual-strength check.</p>",
                    "sources": [
                        {
                            "id": "DESI787-00041",
                            "set": 6,
                ...
                {
                    "id": "check-desi787-00044",
                    "status": "review",
                    "prompt": "The strength of the column with helical reinforcement shall be ______ times the strength of a similar column with lateral ties.",
                ...
                {
                    "id": "check-desi787-00078",
                    "status": "review",
                    "prompt": "The 400 mm x 500 mm square column having both end fixed has unsupported length 5 m. The column falls under ______ Type.",
                ...
                {
                    "id": "check-desi787-00079",
                    "status": "review",
                    "prompt": "When minimum eccentricity in columns does not exceed 0.05 times the lateral dimension, the design axial load carrying capacity is reduced by ______.",
                    "html": "<p><strong>No extra blanket 10% reduction:</strong> §39.3's expression <strong>0.4f<sub>ck</sub>A<sub>c</sub> + 0.67f<sub>y</sub>A<sub>sc</sub></strong> already incorporates its nominal-eccentricity allowance; the cited clause does not prescribe another multiplication by 0.9. </p><p>Moreover, the 400 mm/5 m column has e<sub>min</sub> = <strong>23.333… mm &gt; 20 mm = 0.05D</strong>, so the shortcut fails its eccentricity condition. </p><p>Shortness alone cannot authorize it, nor can the shortcut discard actual bending moments.</p>",
                    "sources": [
                        {
                            "id": "DESI787-00079",
                            "set": 10,
                ...
                {
                    "id": "check-desi787-00080",
                    "status": "review",
                    "prompt": "Minimum eccentricity to be considered for axially loaded RCC column of size 400 mm x 400 mm with unsupported length of 5 m is:",
                    "html": "<p><strong>No extra blanket 10% reduction:</strong> §39.3's expression <strong>0.4f<sub>ck</sub>A<sub>c</sub> + 0.67f<sub>y</sub>A<sub>sc</sub></strong> already incorporates its nominal-eccentricity allowance; the cited clause does not prescribe another multiplication by 0.9. </p><p>Moreover, the 400 mm/5 m column has e<sub>min</sub> = <strong>23.333… mm &gt; 20 mm = 0.05D</strong>, so the shortcut fails its eccentricity condition. </p><p>Shortness alone cannot authorize it, nor can the shortcut discard actual bending moments.</p>",
                    "sources": [
                        {
                            "id": "DESI787-00080",
                ...
                {
                    "id": "check-desi787-00111",
                    "status": "review",
                    "prompt": "Spacing between two longitudinal bars should not be more than",
                ...
                {
                    "id": "check-desi787-00165",
                    "status": "review",
                    "prompt": "The minimum area of longitudinal reinforcement in a RCC column 400 mm x 400 mm, shall not be less than ......",
                ...
                {
                    "id": "check-desi787-00166",
                    "status": "review",
                    "prompt": "The maximum area of longitudinal reinforcement in a RCC column 400 mm x 400 mm, shall not be less than ......",
                ...
                {
                    "id": "check-desi787-00169",
                    "status": "review",
                    "prompt": "In designing rectangular combined footing ...... should be adopted as design value.",
                    "html": "<p>Locating the <strong>load-system resultant</strong> and establishing contact pressure are successive steps, not competing complete designs. </p><p>The maximum-moment items identify an important flexural demand, but shear, punching, anchorage, bearing and settlement remain necessary. </p>",
                    "sources": [
                        {
                            "id": "DESI787-00169",
                            "set": 19,
                ...
                {
                    "id": "check-desi787-00170",
                    "status": "review",
                    "prompt": "The design of rigid rectangular combined footing consist in determining the ......",
                    "html": "<p><strong>Footing design is not one selected quantity:</strong> near-identical stems separately key pressure distribution and the centre of gravity of column loads. </p><p>Locating the <strong>load-system resultant</strong> and establishing contact pressure are successive steps, not competing complete designs. </p><p>Do not confuse the resultant of unequal loads with an unweighted geometric average of column locations.</p>",
                    "sources": [
                        {
                            "id": "DESI787-00170",
                            "set": 19,
                ...
                {
                    "id": "check-desi787-00171",
                    "status": "review",
                    "prompt": "If the resultant of the soil pressure coincides with the resultant of the loads, the soil is pressure is assumed to be ......",
                    "html": "<p><strong>Coincident resultants do not prove uniform contact:</strong> collinearity of the applied and soil-reaction resultants is required by equilibrium for nonuniform pressure too. </p><p>Uniformity is an additional idealization, obtained in the linear full-contact model when the applied resultant passes through the <strong>area centroid</strong>. </p>",
                    "sources": [
                        {
                            "id": "DESI787-00171",
                            "set": 19,
                ...
                {
                    "id": "check-desi787-00174",
                    "status": "review",
                    "prompt": "Minimum thickness of edge of footing is",
                ...
                {
                    "id": "check-desi787-00175",
                    "status": "review",
                    "prompt": "Reinforcement on combined footing is placed at",
                ...
                {
                    "id": "check-desi787-00176",
                    "status": "review",
                    "prompt": "Given size of column as 200 mm x 400 mm carrying 330kN load and soil bearing capacity 150kN/m². Calculate footing area for isolated footing required for such column in square meter.",
                ...
                {
                    "id": "check-desi787-00177",
                    "status": "review",
                    "prompt": "For RC footing ratio of long side to short side 1.5, the ratio of reinforcement in central band width to total reinforcement in short direction is:",
                ...
                {
                    "id": "check-desi787-00178",
                    "status": "review",
                    "prompt": "Cube strength of concrete to be used for pre-tensioned and post- tensioned work should not be less than",
                    "html": "<p><strong>Characteristic grade is not transfer strength.</strong> The cited IS 1343:2012 M40 pre-tensioned/M30 post-tensioned pair concerns characteristic cube strength. It does not prescribe a universal transfer-strength pair for every member.</p>",
                    "sources": [
                        {
                            "id": "DESI787-00178",
                            "set": 20,
                ...
                {
                    "id": "check-desi787-00182",
                    "status": "review",
                    "prompt": "The soffit of the beam after transfer of prestress to concrete will be under",
                ...
                {
                    "id": "check-soil136-00411",
                    "status": "review",
                    "prompt": "The design of rigid rectangular combined footing consist in determining the ______.",
                    "html": "<p><strong>Footing design is not one selected quantity:</strong> near-identical stems separately key pressure distribution and the centre of gravity of column loads. </p><p>Locating the <strong>load-system resultant</strong> and establishing contact pressure are successive steps, not competing complete designs. </p><p>Do not confuse the resultant of unequal loads with an unweighted geometric average of column locations.</p>",
                    "sources": [
                        {
                            "id": "SOIL136-00411",
                ...
                {
                    "id": "check-soil136-00415",
                    "status": "review",
                    "prompt": "If the resultant of the soil pressure coincides with the resultant of the loads, the soil is pressure is assumed to be ______.",
                    "html": "<p><strong>Coincident resultants do not prove uniform contact:</strong> collinearity of the applied and soil-reaction resultants is required by equilibrium for nonuniform pressure too. </p><p>Uniformity is an additional idealization, obtained in the linear full-contact model when the applied resultant passes through the <strong>area centroid</strong>. </p>",
                    "sources": [
                        {
                            "id": "SOIL136-00415",
                ...
                {
                    "id": "check-soil136-00416",
                    "status": "review",
                    "prompt": "In designing rectangular combined footing ______ should be adopted as the design value.",
                    "html": "<p>Locating the <strong>load-system resultant</strong> and establishing contact pressure are successive steps, not competing complete designs. </p><p>The maximum-moment items identify an important flexural demand, but shear, punching, anchorage, bearing and settlement remain necessary. </p>",
                    "sources": [
                        {
                            "id": "SOIL136-00416",
                ...
                {
                    "id": "check-soil136-00417",
                    "status": "review",
                    "prompt": "Generally combined footing is assumed to rest on ______.",
                    "html": "<p><strong>Uniform support pressure is an additional idealization.</strong> Homogeneous soil and equilibrium of resultants do not prove uniform contact. The linear full-contact model gives uniform pressure only when the load resultant passes through the footing-area centroid.</p>",
                    "sources": [
                        {
                            "id": "SOIL136-00417",
                            "set": 32,
                ...
                {
                    "id": "check-soil136-00418",
                    "status": "review",
                    "prompt": "The design procedure of strap footing is essentially that of ______.",
                ...
                {
                    "id": "check-soil136-00419",
                    "status": "review",
                    "prompt": "What will be the shape of footing if a=0, is substituted in the pressure intensity equation “q2 = α.qs”?",
                ...
                {
                    "id": "check-soil136-00430",
                    "status": "review",
                    "prompt": "The weight of the raft is not considered in the structural design, because",
                    "html": "<p><strong>Raft simplifications have limits:</strong> self-weight cancels out of a net structural bending calculation only to the extent that its downward distribution is matched by the corresponding upward pressure component in a compatible model. </p><p>It remains in gross bearing and dead-load checks. </p>",
                    "sources": [
                        {
                            "id": "SOIL136-00430",
                            "set": 33,
                ...
                {
                    "id": "check-soil136-00433",
                    "status": "review",
                    "prompt": "In raft footing, if the C.G of the load coincide with the centroid of the raft, the upward load is considered as",
                    "html": "<p><strong>Coincident resultants do not prove uniform contact:</strong> collinearity of the applied and soil-reaction resultants is required by equilibrium for nonuniform pressure too. </p><p>Uniformity is an additional idealization, obtained in the linear full-contact model when the applied resultant passes through the <strong>area centroid</strong>. </p><p>The raft item includes that centroid condition, but still assumes a pressure model; homogeneous soil alone does not make actual contact pressure uniform.</p>",
                    "sources": [
                        {
                            "id": "SOIL136-00433",
                            "set": 33,
                ...
                {
                    "id": "check-soil136-00434",
                    "status": "review",
                    "prompt": "Raft is subdivided in to series of beams to establish",
                    "html": "<p><strong>Raft simplifications have limits:</strong> self-weight cancels out of a net structural bending calculation only to the extent that its downward distribution is matched by the corresponding upward pressure component in a compatible model. </p><p>Strip subdivision approximates two-way behaviour and needs compatible load sharing and foundation–soil stiffness assumptions; it generates <strong>shear-force</strong> diagrams, not a diagram that by itself establishes shear failure.</p>",
                    "sources": [
                        {
                            "id": "SOIL136-00434",
                            "set": 33,
