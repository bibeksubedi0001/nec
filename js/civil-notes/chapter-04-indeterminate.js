(function () {
    ...
    Object.assign(registry, {
        ACiE0406: {
            code: "ACiE0406",
            questionCount: 58,
            formulaSheet: "<table><thead><tr><th scope='col'>Quantity</th><th scope='col'>Relation and basis</th></tr></thead><tbody><tr><th scope='row'>Flexibility (force) method</th><td>Δ<sub>i</sub><sup>0</sup> + Σf<sub>ij</sub>X<sub>j</sub> = Δ<sub>i</sub><sup>s</sup>; f<sub>ij</sub> = ∫m<sub>i</sub>m<sub>j</sub>/(EI) ds.</td></tr><tr><th scope='row'>Flexibility and stiffness</th><td>f = K<sup>−1</sup>; both matrices are symmetric.</td></tr><tr><th scope='row'>Propped cantilever, UDL</th><td>Prop reaction R = 3wL/8; contraflexure at x = L/4 from the fixed end.</td></tr><tr><th scope='row'>Two-hinged parabolic arch</th><td>UDL horizontal thrust H = wL<sup>2</sup>/(8h).</td></tr><tr><th scope='row'>Slope-deflection</th><td>M<sub>AB</sub> = M<sub>AB</sub><sup>F</sup> + (2EI/L)(2θ<sub>A</sub> + θ<sub>B</sub> − 3ψ); chord rotation ψ = (v<sub>A</sub> − v<sub>B</sub>)/L.</td></tr><tr><th scope='row'>Moment distribution</th><td>Distribution factor DF = k/Σk; carry-over factor ½.</td></tr><tr><th scope='row'>Plastic moments</th><td>M<sub>y</sub> = f<sub>y</sub>Z<sub>e</sub>; M<sub>p</sub> = f<sub>y</sub>Z<sub>p</sub>; shape factor α = M<sub>p</sub>/M<sub>y</sub> = Z<sub>p</sub>/Z<sub>e</sub> (1.5 for a rectangle).</td></tr><tr><th scope='row'>Collapse loads</th><td>SS point P = 4M<sub>p</sub>/L; SS UDL w = 8M<sub>p</sub>/L<sup>2</sup>; fixed-fixed central W = 8M<sub>p</sub>/L.</td></tr><tr><th scope='row'>Load factor</th><td>λ<sub>c</sub> = P<sub>collapse</sub>/P<sub>working</sub>.</td></tr></tbody></table>",
            blocks: [
                {
                    id: "indeterminacy-and-method-selection",
                    title: "Restraints, redundants and method selection",
                    html: "<p>A <strong>continuous beam</strong> extends over more than two supports with flexural continuity between its spans. An intermediate vertical support does not automatically insert a hinge into the beam: adjacent member-end moments can be nonzero while balancing at the joint. Actual hinges, releases and support directions must be identified before counting indeterminacy.</p><table><thead><tr><th scope='col'>Method family</th><th scope='col'>Primary unknowns</th><th scope='col'>Equations solved</th></tr></thead><tbody><tr><th scope='row'>Force or flexibility</th><td>Independent redundant forces and moments</td><td>Compatibility of the released movements</td></tr><tr><th scope='row'>Displacement or stiffness</th><td>Independent joint translations and rotations</td><td>Equilibrium at the free coordinates</td></tr></tbody></table>",
                    moreHtml: "<p>Both families must ultimately satisfy <strong>equilibrium, compatibility and the force-deformation law</strong>. The degree of static indeterminacy, DSI, counts independent redundants; the degree of kinematic indeterminacy, DKI, counts independent movements after restraints and modelling assumptions. A force method is often economical when DSI &lt; DKI, but convenient releases and simple integrations matter too. The stiffness method is not a force method merely because it eventually calculates reactions.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>Release r independent redundants to obtain a <strong>stable primary structure</strong>, not a mechanism. Choose a positive direction for each redundant X<sub>j</sub> and its work-conjugate movement Δ<sub>j</sub>. Compute the load-only movements Δ<sub>i</sub><sup>0</sup>, then impose one unit redundant at a time.</p><p><strong>Δ<sub>i</sub><sup>0</sup> + Σ f<sub>ij</sub>X<sub>j</sub> = Δ<sub>i</sub><sup>s</sup></strong>, for i = 1 to r. Here Δ<sub>i</sub><sup>s</sup> is the prescribed support or release movement, usually zero. Thus r independent redundants require r independent compatibility equations.</p><p>The coefficient <strong>f<sub>ij</sub></strong> is movement at coordinate i caused by a unit generalized force at j, with the other independent generalized forces zero. The resulting movements form <strong>column j</strong> of the flexibility matrix; the other movements are not held at zero. A rotational coordinate is excited by a unit couple, not a unit transverse force.</p><p>For small, linear-elastic bending deformation, <strong>f<sub>ij</sub> = ∫ m<sub>i</sub>m<sub>j</sub> ds/(EI)</strong> over all members, where m<sub>i</sub> is bending moment per unit action at i. Add axial or shear-deformation contributions when those effects are retained. </p>",
                    moreHtml: "<p>Settlement, temperature and fabrication errors enter the compatibility balance rather than being invented as extra applied loads.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>For a <strong>linear conservative elastic system</strong> with consistent work-conjugate coordinates, reciprocal work gives f<sub>ij</sub> = f<sub>ji</sub>; the corresponding stiffness matrix is symmetric too. This is a statement about the mechanical model and coordinate definitions, not an unconditional property of every nonlinear or nonconservative analysis.</p><p><strong>f = K<sup>−1</sup></strong> applies only to invertible force-displacement maps on the <strong>same independent coordinates with the same restraints</strong>. A redundant-force flexibility matrix is not automatically the inverse of a global nodal stiffness matrix. </p><table><thead><tr><th scope='col'>Coordinate pair</th><th scope='col'>Stiffness</th><th scope='col'>Flexibility</th></tr></thead><tbody><tr><th scope='row'>Force / translation</th><td>N/m</td><td>m/N</td></tr><tr><th scope='row'>Moment / rotation</th><td>N·m/rad</td><td>rad/(N·m)</td></tr></tbody></table>",
                    moreHtml: "<p>An unrestrained beam has rigid-body modes and a singular stiffness matrix. Restrain those modes and check for mechanisms before solving; a stable elastic reduced system is positive definite when every remaining nonzero movement stores strain energy.</p><p>Mixed translation-rotation matrices also contain coupling coefficients with different force-length dimensions. Radians are dimensionless in dimensional analysis but must be used numerically, not degrees. Changing metres to millimetres or scaling rotations requires consistent scaling of their conjugate forces; otherwise numerical symmetry and comparisons can be misleading.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>For a member AB, take M<sub>AB</sub> and M<sub>BA</sub> as moments <strong>applied to the member</strong> at A and B, both positive clockwise. Joint rotations θ<sub>A</sub>, θ<sub>B</sub> and chord rotation ψ are also clockwise-positive, in radians. </p><p><strong>M<sub>AB</sub> = M<sub>AB</sub><sup>F</sup> + (2EI/L)(2θ<sub>A</sub> + θ<sub>B</sub> − 3ψ)</strong>.</p><p><strong>M<sub>BA</sub> = M<sub>BA</sub><sup>F</sup> + (2EI/L)(2θ<sub>B</sub> + θ<sub>A</sub> − 3ψ)</strong>.</p><table><thead><tr><th scope='col'>Load on horizontal AB</th><th scope='col'>Left fixed-end moment</th><th scope='col'>Right fixed-end moment</th></tr></thead><tbody><tr><th scope='row'>Downward full UDL w</th><td>−wL<sup>2</sup>/12</td><td>+wL<sup>2</sup>/12</td></tr><tr><th scope='row'>Downward central point load P</th><td>−PL/8</td><td>+PL/8</td></tr></tbody></table><p>The superscript F denotes the load-only moments with both end rotations and translations restrained. After inserting actual support conditions, solve <strong>joint equilibrium</strong> for the unknown rotations and sway. With these member-action signs, ΣM<sub>jk</sub> equals the external clockwise joint couple C<sub>j</sub>; at a joint without a couple the sum is zero. </p>",
                    moreHtml: "<p>Use small-displacement, prismatic Euler-Bernoulli bending with constant EI and neglect axial and shear deformation in these equations.</p><p>Frame sway also requires translational equilibrium. Do not mix these end-action signs with sagging-positive section-moment diagrams without conversion.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>Four successive supports A, B, C and D, with no overhangs or extra internal releases, define <strong>three spans</strong>: AB, BC and CD. Each span has two end-moment expressions, giving <strong>six slope-deflection equations and six member-end moment quantities</strong>. That does not establish six independent displacement unknowns.</p><table><thead><tr><th scope='col'>All vertical support translations held</th><th scope='col'>Independent rotations before condensation</th></tr></thead><tbody><tr><th scope='row'>A and D fixed; beam continuous at B and C</th><td>θ<sub>B</sub>, θ<sub>C</sub>: two; θ<sub>A</sub> = θ<sub>D</sub> = 0.</td></tr><tr><th scope='row'>A and D simple ends; beam continuous at B and C</th><td>θ<sub>A</sub>, θ<sub>B</sub>, θ<sub>C</sub>, θ<sub>D</sub>: four, with zero end moments at A and D.</td></tr></tbody></table><p>The simple-end rotations can be eliminated using their zero-moment conditions, leaving two interior rotations in a condensed formulation. At an intermediate support, M<sub>BA</sub> + M<sub>BC</sub> = 0 without an external couple; this does not require either term individually to vanish. </p>",
                    moreHtml: "<p>Add independent sway translations only when the frame permits them. Known settlements are prescribed displacements, while genuinely fixed-end rotations are known zeros.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>Define k<sub>A</sub> = M<sub>A</sub>/θ<sub>A</sub> for an unloaded prismatic member of length L. In the following table, <strong>A is held against transverse translation</strong>. The far-end conditions specify which other coordinates are held and which are allowed to respond.</p><table><thead><tr><th scope='col'>Conditions at B</th><th scope='col'>Near-end rotational stiffness</th></tr></thead><tbody><tr><th scope='row'>Translation and rotation fixed</th><td>4EI/L</td></tr><tr><th scope='row'>Translation fixed; moment zero, rotation free</th><td>3EI/L</td></tr><tr><th scope='row'>Rotation fixed; transverse translation free with zero transverse force</th><td>EI/L</td></tr></tbody></table><p>The second result follows by setting M<sub>BA</sub> = 0, which gives θ<sub>B</sub> = −θ<sub>A</sub>/2 when ψ = 0. The third is a specifically defined <strong>rotation-restrained sliding guide</strong>, not a universal interpretation of the word guided. </p><p>If B is fixed but <strong>A can also translate freely</strong>, a real cantilever tip couple instead gives |θ<sub>A</sub>| = |M<sub>A</sub>|L/(EI), so the tip moment/rotation ratio is EI/L, not 4EI/L. E is elastic modulus, I is second moment of area, and EI/L has moment units per radian; E/L is dimensionally wrong. For EI = 20000 kN·m<sup>2</sup> and L = 5 m, the three tabulated stiffnesses are 16000, 12000 and 4000 kN·m/rad respectively.</p>",
                    moreHtml: "<p>A fully free far end can follow a near-end rotation as a rigid link and supplies no rotational stiffness under this test.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>Moment distribution iteratively solves the joint-equilibrium equations underlying slope-deflection, principally for indeterminate beams and frames. Lock the unknown joint rotations, calculate load-only end moments, and then release and balance one joint at a time while keeping the other working restraints explicit.</p><ol><li>At joint j, compute clockwise unbalance U<sub>j</sub> = ΣM<sub>jk</sub> − C<sub>j</sub>.</li><li>For each connected active member, calculate <strong>DF<sub>jk</sub> = k<sub>jk</sub>/Σk<sub>j</sub></strong>. The distribution factors sum to 1 when the total stiffness is nonzero.</li><li>Apply the <strong>opposite</strong> balancing increment: ΔM<sub>jk</sub> = −DF<sub>jk</sub>U<sub>j</sub>.</li><li>Carry each increment to its far end using that member's actual restraint; repeat until joint residuals are negligible.</li></ol><p><em>Complete one-joint example:</em> AB and BC each have L = 4 m and equal EI; A is fixed, C is a simple end, and all support translations are held. A downward 8 kN/m UDL acts only on AB. Initially M<sub>AB</sub> = −10.667 and M<sub>BA</sub> = +10.667 kN·m; the unloaded BC moments are zero. Using 4EI/L for BA and the condensed 3EI/L for BC gives <strong>DF<sub>BA</sub> = 4/7, DF<sub>BC</sub> = 3/7</strong>.</p><p>The balancing increments at B are −6.095 and −4.571 kN·m. Half of the BA increment carries to fixed A; none carries to simple C. Final moments are <strong>M<sub>AB</sub> = −13.714, M<sub>BA</sub> = +4.571, M<sub>BC</sub> = −4.571, M<sub>CB</sub> = 0 kN·m</strong>. </p>",
                    moreHtml: "<p>Only B has an independent rotation here, so this balance completes the solution. A sway frame additionally needs its translation and storey-force equilibrium; locking sway forever solves a different structure.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p><em>Bank extension to the listed analysis methods:</em> a basic lateral-load portal approximation often places inflection points at mid-height of columns and mid-span of beams. These assumed zero-moment sections simplify equilibrium calculations; they are not physical hinges or exact positions guaranteed by the word portal.</p><p>For a column with no transverse distributed loading, shear is constant and its moment diagram is linear. If opposing end-moment magnitudes are m<sub>b</sub> at the base and m<sub>t</sub> at the top, the zero lies <strong>a = Lm<sub>b</sub>/(m<sub>b</sub> + m<sub>t</sub>)</strong> from the base. Equal magnitudes give L/2. The assumed <strong>L/3</strong> location corresponds to m<sub>t</sub> = 2m<sub>b</sub>, not to every partially fixed support.</p><p><em>Illustration:</em> a 3 m column with opposing 20 and 40 kN·m end moments has its zero 1 m from the base. Actual partial fixity should be represented by the support's moment-rotation relation, such as a rotational spring, and solved with the rest of the frame. </p>",
                    moreHtml: "<p>Gravity loading, unequal EI, unequal spans and different joint restraints can move or remove the assumed points of inflection.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p><em>Bank extension:</em> classical column analogy is a <strong>force or flexibility method</strong>. The member centreline is represented by an analogous section whose strip weight is ds/(EI). </p>",
                    moreHtml: "<p>The load-only bending-moment diagram supplies the analogous loading, and an analogous axial-plus-bending stress distribution supplies a redundant-moment correction. This is a mathematical analogy, not a column buckling calculation.</p><p>For appropriate planar released frames, a correction moment can be represented by <strong>a + bx + cy</strong>, with the coefficient units chosen so each term is a moment. The three coefficients are determined by weighted compatibility using the corresponding unit-moment patterns. This explains the familiar <strong>up-to-three-redundant elementary implementation</strong>; it does not prove that mathematics forbids larger flexibility systems.</p><p>Choose a stable release system and define its coordinates before using the analogy. More complicated structures can require additional independent compatibility equations, generalized force methods or direct stiffness assembly. Slope-deflection, moment distribution and Kani's rotation-contribution method instead belong to the displacement family because their working unknowns represent joint movements.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>In elastic pure bending of a homogeneous section, stress varies linearly with distance from the centroidal neutral axis. The <strong>yield moment M<sub>y</sub> = f<sub>y</sub>Z<sub>e</sub> = f<sub>y</sub>I/c</strong> first brings an extreme fibre to yield stress f<sub>y</sub>; c is the governing extreme-fibre distance. </p><p>For ideal fully plastic pure bending with uniform and equal tensile/compressive yield stress, <strong>M<sub>p</sub> = f<sub>y</sub>Z<sub>p</sub></strong>, where Z<sub>p</sub> = ∫|y| dA and y is measured from the plastic neutral axis, PNA. Force equilibrium requires f<sub>y</sub>A<sub>c</sub> = f<sub>y</sub>A<sub>t</sub>, so <strong>A<sub>c</sub> = A<sub>t</sub></strong>. </p><p>A symmetry axis aligned with the bending neutral-axis direction also bisects the area, so the two descriptions can overlap. With axial compression N taken positive, the fully plastic force balance becomes <strong>f<sub>y</sub>(A<sub>c</sub> − A<sub>t</sub>) = N</strong>; unequal yield stresses likewise require a strength-weighted balance rather than equal areas. </p>",
                    moreHtml: "<p>It does not mean every fibre has yielded. The elastic core shrinks as further moment produces yielding through more of the depth.</p><p>The PNA bisects area under these assumptions; it need not coincide with the elastic centroidal axis of an unsymmetrical section.</p><p>Neither an axial load nor a composite section may be silently inserted into the equal-area rule.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>The <strong>shape factor α = M<sub>p</sub>/M<sub>y</sub> = Z<sub>p</sub>/Z<sub>e</sub></strong> measures section reserve from first yield to ideal full plastic bending, not whole-frame redistribution. </p><table><thead><tr><th scope='col'>Section and axis</th><th scope='col'>Z<sub>e</sub></th><th scope='col'>Z<sub>p</sub></th><th scope='col'>α</th></tr></thead><tbody><tr><th scope='row'>Rectangle, axis parallel to b</th><td>bd<sup>2</sup>/6</td><td>bd<sup>2</sup>/4</td><td>1.5</td></tr><tr><th scope='row'>Solid circle, diameter axis</th><td>πd<sup>3</sup>/32</td><td>d<sup>3</sup>/6</td><td>16/(3π) ≈ 1.698</td></tr><tr><th scope='row'>Typical rolled I, major axis</th><td>I/c</td><td>First moments of the two plastic stress areas</td><td>About 1.10–1.20</td></tr></tbody></table><p>For general pure bending, Z<sub>p</sub> = A<sub>c</sub>y<sub>c</sub> + A<sub>t</sub>y<sub>t</sub>, using positive centroid distances from the PNA. I-section flanges already lie near the extreme fibres, so their extra reserve is relatively small. Actual flange/web proportions and the bending axis determine the ratio. A solid circle is <strong>not</strong> an exception with Z<sub>p</sub> &lt; Z<sub>e</sub>.</p><p><strong>Bank rectangle:</strong> b = 200 mm and d = 400 mm give Z<sub>p</sub> = 200 × 400<sup>2</sup>/4 = <strong>8 × 10<sup>6</sup> mm<sup>3</sup></strong>; Z<sub>e</sub> is only 5.333 × 10<sup>6</sup> mm<sup>3</sup>. Do not substitute the elastic modulus when plastic capacity is requested.</p><p><strong>Bank stress calculation:</strong> 1 kN·m = 10<sup>6</sup> N·mm. Thus M<sub>p</sub> = 120 kN·m and Z<sub>p</sub> = 5 × 10<sup>4</sup> mm<sup>3</sup> give <strong>f<sub>y</sub> = 120 × 10<sup>6</sup>/(5 × 10<sup>4</sup>) = 2400 N/mm<sup>2</sup></strong>, not 240. The supplied shape factor 1.2 is irrelevant because Z<sub>p</sub> is already given. </p>",
                    moreHtml: "<p>In the following table, b is rectangular width, d is bending depth or circle diameter, and the section is homogeneous with equal tensile/compressive yield stress and no axial force.</p><p>The corrected option follows the printed inputs. An unusually high inferred yield stress still calls for checking the source data and material specification; it is not a reason to silently change an exponent.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>Elementary rigid-plastic collapse analysis concentrates incremental inelastic rotations at plastic hinges and treats the intervening lengths as <strong>rigid links during the mechanism motion</strong>. It does not assert that those lengths were perfectly rigid throughout the earlier elastic loading. Existing real pins contribute no plastic moment resistance.</p><p>For an admissible small mechanism displacement, <strong>ΣP<sub>i</sub>δ<sub>i</sub> + ∫wδ dx + ΣC<sub>i</sub>θ<sub>i</sub> = ΣM<sub>p,j</sub>|Δφ<sub>j</sub>|</strong>. The left side is external work by forces, distributed loads and applied couples; Δφ<sub>j</sub> is the <strong>relative rotation across hinge j</strong>. Moment times rotation and force times displacement both have work units. If two adjoining links rotate oppositely by θ, their hinge jump is 2θ, not θ.</p><table><thead><tr><th scope='col'>Collapse check</th><th scope='col'>Required condition</th></tr></thead><tbody><tr><th scope='row'>Equilibrium</th><td>Loads, reactions and internal actions balance.</td></tr><tr><th scope='row'>Yield bound</th><td>|M(x)| ≤ M<sub>p</sub>(x) everywhere.</td></tr><tr><th scope='row'>Mechanism</th><td>The hinge pattern permits a compatible nonzero collapse movement.</td></tr></tbody></table><p>A statically admissible moment field within the yield bounds supplies a <strong>lower bound</strong> on collapse load; a kinematically admissible mechanism supplies an <strong>upper bound</strong>. Agreement establishes the exact ideal result. Counting r + 1 new hinges is only a shortcut for a suitable complete bending mechanism, not a substitute for examining local mechanisms and existing releases. </p>",
                    moreHtml: "<p>Adequate ductility and rotation capacity, stable braced sections, and absence of premature buckling or unmodelled axial interaction are essential assumptions.</p>",
                    sources: [
                ...
                },
                {
                    ...
                    html: "<p>For a fixed loading pattern, the structural collapse load factor is <strong>λ<sub>c</sub> = P<sub>coll</sub>/P<sub>work</sub></strong>. The section shape factor α = M<sub>p</sub>/M<sub>y</sub> is a different ratio. Under an assumed working-stress limit σ<sub>allow</sub>, define n = f<sub>y</sub>/σ<sub>allow</sub>. If the section is stressed to that limit, M<sub>work</sub> = M<sub>y</sub>/n, so its <strong>section moment reserve M<sub>p</sub>/M<sub>work</sub> = αn</strong>.</p><p>That product becomes a structural load ratio only when the relevant moment-to-load relationship makes the identification valid. A frame may redistribute moments and form another mechanism. For example, the full-UDL propped cantilever has elastic fixed-end moment magnitude wL<sup>2</sup>/8, but plastic collapse intensity (6 + 4√2)M<sub>p</sub>/L<sup>2</sup>. If its working load is limited by the elastic fixed-end stress, <strong>λ<sub>c</sub> = [(6 + 4√2)/8]αn</strong>, not simply αn.</p><p><strong>Bank arithmetic under its stated stress assumption:</strong> α = 1.12 and n = 1.5 give section reserve 1.68. Increasing the allowable stress by 20% increases the corresponding allowable section moment by 1.2, so the revised reserve is <strong>1.12 × 1.5/1.2 = 1.4</strong>. </p>",
                    moreHtml: "<p>This is an illustration of the supplied working-stress rule, <strong>not a current wind or earthquake code prescription</strong>, nor permission to increase allowable stresses in an actual project without the applicable design standard.</p>",
                    sources: [
            ...
            cautions: [
                {
                    "id": "check-stru935-00019",
                    "status": "review",
                    "prompt": "In case of propped cantilever beam, the internal hinge is located at what distance from propped end",
                ...
                {
                    "id": "check-stru935-00043",
                    "status": "review",
                    "prompt": "In case of a simply supported beam carrying a concentrated load at the mid span, what will be the length of the plastic hinge formed?",
                ...
                {
                    "id": "check-stru935-00066",
                    "status": "review",
                    "prompt": "Stiffness of the end A if the far end B is vertical guided roller is ______",
                    "html": "<p><strong>Missing I and ambiguous end restraints:</strong> the E/L options omit the second moment of area. </p><p>The words guided roller or free end alone cannot settle these questions.</p>",
                    "sources": [
                        {
                            "id": "STRU935-00066",
                            "set": 7,
                ...
                {
                    "id": "check-stru935-00069",
                    "status": "review",
                    "prompt": "For a fixed-end beam of length L and central point load of W, what will be the value of W at collapse?",
                    "html": "<p><strong>Fixed-fixed central-load contradiction:</strong> the 6M<sub>p</sub>/L item counts the middle hinge rotation as θ instead of 2θ. </p><p>Correct virtual work is W(θL/2) = M<sub>p</sub>(θ + 2θ + θ), giving <strong>W<sub>coll</sub> = 8M<sub>p</sub>/L</strong>. </p><p>The separate 8M<sub>p</sub>/L and WL/8 keys have correct results but their explanations use dimensionally incomplete work expressions involving WL/4 and missing rotation factors. </p>",
                    "sources": [
                        {
                            "id": "STRU935-00069",
                            "set": 8,
                ...
                {
                    "id": "check-stru935-00071",
                    "status": "review",
                    "prompt": "Stiffness for free end if far end is fixed.",
                    "html": "<p><strong>“Free end” is ambiguous.</strong> A truly free-to-translate cantilever tip has moment/rotation stiffness EI/L. The familiar 4EI/L requires restrained end translations; an E/L option also omits section inertia.</p>",
                    "sources": [
                        {
                            "id": "STRU935-00071",
                            "set": 8,
                ...
                {
                    "id": "check-stru935-00073",
                    "status": "review",
                    "prompt": "For a two hinged arch having constant EI, the horizontal thrust H at a point (x, y) on the arch, in terms of beam moment M is given by",
                ...
                {
                    "id": "check-desi787-00062",
                    "status": "review",
                    "prompt": "For an I-beam, shape factor is 1.12. The factor of safety in bending is 1.5. If the allowable stress is increased by 20% for wind and earthquake loads, then the load factor is",
                    "html": "<p><strong>Section reserve is not automatically frame collapse:</strong> shape factor times bending factor of safety gives M<sub>p</sub>/M<sub>work</sub> when the section is at the assumed allowable stress. </p><p>The 20% stress-increase example evaluates to <strong>1.4</strong>, but its arithmetic supplies neither a universal frame load factor nor a current wind/seismic design rule.</p>",
                    "sources": [
                        {
                            "id": "DESI787-00062",
                            "set": 8,
                ...
                {
                    "id": "check-stru935-00102",
                    "status": "review",
                    "prompt": "To draw qualitative ILD of indeterminate structure, which of the following concept is used.",
                    "html": "<p>Curvature with changing load position does not imply nonlinear response to load magnitude. </p><p>Müller-Breslau is already represented in the bank; its qualitative released shape becomes quantitative only with the correct unit movement and elastic normalization.</p>",
                    "sources": [
                        {
                            "id": "STRU935-00102",
                            "set": 11,
                ...
                {
                    "id": "check-stru935-00103",
                    "status": "review",
                    "prompt": "Where is point of inflection assumed in column and beam of a portal frame (length of column and beam is L)?",
                    "html": "<p><strong>Portal fractions are assumptions:</strong> midpoint inflection is a common approximate lateral-load portal rule. </p><p>Actual end stiffnesses, joint restraints and loading determine the moment zeros; introducing a physical hinge there would change the structure.</p>",
                    "sources": [
                        {
                            "id": "STRU935-00103",
                            "set": 11,
                ...
                {
                    "id": "check-stru935-00111",
                    "status": "review",
                    "prompt": "ILD of statically indeterminate beams consist of:",
                    "html": "<p><strong>Influence-line qualifications:</strong> indeterminate-beam influence lines are generally piecewise curves, but zero or straight portions can occur in particular responses or regions. </p>",
                    "sources": [
                        {
                            "id": "STRU935-00111",
                            "set": 11,
                ...
                {
                    "id": "check-stru935-00113",
                    "status": "review",
                    "prompt": "For a particular steel section, which of the following options represents a product of the factor of safety and shape factor",
                    "html": "<p><strong>Section reserve is not automatically frame collapse:</strong> shape factor times bending factor of safety gives M<sub>p</sub>/M<sub>work</sub> when the section is at the assumed allowable stress. </p>",
                    "sources": [
                        {
                            "id": "STRU935-00113",
                            "set": 12,
                ...
                {
                    "id": "check-stru935-00116",
                    "status": "review",
                    "prompt": "A simply supported beam of span 6 m carries an UDL of 25kN/m. The plastic moment capacity is",
                ...
                {
                    "id": "check-stru935-00117",
                    "status": "corrected",
                    "prompt": "For a fixed beam of length L and centrally loaded point load of W, what will be the value of W at collapse",
                    "html": "<p><strong>Corrected: W = 8M<sub>p</sub>/L, option B.</strong> The central plastic hinge rotates twice as much as either support hinge, so internal work is 4M<sub>p</sub>θ.</p>",
                    "sources": [
                        {
                            "id": "STRU935-00117",
                            "set": 12,
                ...
                {
                    "id": "check-stru935-00118",
                    "status": "review",
                    "prompt": "For a fixed beam of length L and centrally loaded point load of W, what will be the plastic moment capacity",
                    "html": "<p>Correct virtual work is W(θL/2) = M<sub>p</sub>(θ + 2θ + θ), giving <strong>W<sub>coll</sub> = 8M<sub>p</sub>/L</strong>. </p><p>The separate 8M<sub>p</sub>/L and WL/8 keys have correct results but their explanations use dimensionally incomplete work expressions involving WL/4 and missing rotation factors. </p><p>The elastic coefficient WL/8 does not equate first yield with full plastic capacity.</p>",
                    "sources": [
                        {
                            "id": "STRU935-00118",
                            "set": 12,
                ...
                {
                    "id": "check-stru935-00119",
                    "status": "corrected",
                    "prompt": "The plastic modulus of a section is 5 × 10⁴ mm³. Its shape factor is 1.2 and the plastic moment capacity is 120 kNm, what is the value of the yield stress of the material?",
                    "html": "<p><strong>Corrected: 2400 N/mm<sup>2</sup>, option A.</strong> M<sub>p</sub>/Z<sub>p</sub> uses 120 × 10<sup>6</sup> N mm divided by 5 × 10<sup>4</sup> mm<sup>3</sup>. The printed inputs, not a typical material value, control this calculation.</p>",
                    "sources": [
                        {
                            "id": "STRU935-00119",
                            "set": 12,
                ...
                {
                    "id": "check-stru935-00125",
                    "status": "review",
                    "prompt": "Flexibility matrix is always:",
                ...
                {
                    "id": "check-stru935-00129",
                    "status": "review",
                    "prompt": "Stiffness of the end A if the far end B is fixed is",
                    "html": "<p><strong>Fixed far end:</strong> near-end rotational stiffness is 4EI/L when transverse translations are restrained. A formula containing E/L without I is dimensionally incomplete.</p>",
                    "sources": [
                        {
                            "id": "STRU935-00129",
                            "set": 13,
                ...
                {
                    "id": "check-stru935-00130",
                    "status": "review",
                    "prompt": "Stiffness of the end A if the far end B is hinged is",
                    "html": "<p><strong>Hinged far end:</strong> near-end rotational stiffness is 3EI/L when transverse translations are restrained. The release changes stiffness and carry-over; I must remain in the expression.</p>",
                    "sources": [
                        {
                            "id": "STRU935-00130",
                            "set": 13,
                ...
                {
                    "id": "check-stru935-00133",
                    "status": "review",
                    "prompt": "In slope deflection method, if left support goes down and right support goes up, then this will be:",
                ...
                {
                    "id": "check-stru935-00137",
                    "status": "review",
                    "prompt": "Initially, how many total unknowns will be there if 4 supports are there using slope deflection method?",
                ...
                {
                    "id": "check-stru935-00140",
                    "status": "review",
                    "prompt": "As per the method adopted for calculation of stiffness matrix, if end A is given translational displacement in a vertically upward direction, the end forces generated at the end \"B\" are",
                ...
                {
                    "id": "check-stru935-00144",
                    "status": "review",
                    "prompt": "Carryover moment at fixed end of member when moment'M' is applied at one end of propped cantilever beam.",
                ...
                {
                    "id": "check-stru935-00146",
                    "status": "review",
                    "prompt": "Carryover moment at fixed end of member when moment'M' is applied at one end of cantilever beam.",
                ...
                {
                    "id": "check-stru935-00148",
                    "status": "review",
                    "prompt": "Which of the following divides section into equal half area?",
                ...
                {
                    "id": "check-stru935-00149",
                    "status": "review",
                    "prompt": "For a particular steel section, which of the following options represents a ratio of the ultimate collapse load to the working load",
                    "html": "<p><strong>Section reserve is not automatically frame collapse:</strong> shape factor times bending factor of safety gives M<sub>p</sub>/M<sub>work</sub> when the section is at the assumed allowable stress. </p><p>The structural collapse/working-load ratio can also depend on redistribution and the governing mechanism. </p>",
                    "sources": [
                        {
                            "id": "STRU935-00149",
                            "set": 15,
                ...
                {
                    "id": "check-stru935-00162",
                    "status": "review",
                    "prompt": "The degree of static indeterminacy up to which column analogy method can be used is",
                ...
                {
                    "id": "check-stru935-00197",
                    "status": "review",
                    "prompt": "At what distance from base is point of inflection assumed in column of a portal frame which is supported by partially fixed supports (length of column is L)?",
                    "html": "<p><strong>Portal fractions are assumptions:</strong> midpoint inflection is a common approximate lateral-load portal rule. </p><p>L/3 from a partially fixed base is another assumed end-moment pattern, not a universal consequence of partial fixity. </p>",
                    "sources": [
                        {
                            "id": "STRU935-00197",
                            "set": 19,
                ...
                {
                    "id": "check-desi787-00197",
                    "status": "review",
                    "prompt": "The plastic section modulus is less than elastic section modulus in",
